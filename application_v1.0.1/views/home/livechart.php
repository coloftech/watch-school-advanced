			...
			<?php foreach ($livechart as $key): ?>
				<div class="col-md-3"  style="border:solid 1px #e5e5e5;height:350px;overflow:auto;padding:5px;">
					...
					<?php 
						...
						$expired_onD = date('Y/m/d',strtotime($key->expired_on));

						$expired_onT = date('H:i:s',strtotime($key->expired_on));

						if($currentD == $expired_onD && $currentT > $expired_onT){
							echo '<span>Now showing</span>';
						}else{
							?>
						<span class="countdown"  value='<?=isset($key->expired_on) ? $key->expired_on : "";?>'></span>
							<?php
						}
					 ?>
					 	<?php $slug = $this->auto_m->getLatest($key->video_id); ?>
					 <a class="btn btn-default btn-sm pull-right" href="<?=site_url('watch/v/'.$slug)?>" titl="Go to latest episode"><i class="fa fa-sign-out"></i></a>
